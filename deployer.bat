...
echo [INFO] Sync marque et logos club vers site...
copy /Y "%~dp0brand-tokens.css" "%SOURCE%\assets\css\brand-tokens.css" >nul
copy /Y "%~dp0brand-icon.svg" "%SOURCE%\assets\icons\icon-192.svg" >nul
copy /Y "%~dp0brand-icon.svg" "%SOURCE%\assets\icons\icon-512.svg" >nul
if not exist "%SOURCE%\assets\brand\logos" mkdir "%SOURCE%\assets\brand\logos"
...
powershell -NoProfile -Command "$bv='%LOGTS%'; $dest='%DEST%'; $files=@((Join-Path $dest 'apps\formations\index.html'),(Join-Path $dest 'apps\materiel\index.html'),(Join-Path $dest 'apps\infos-club\index.html'),(Join-Path $dest 'apps\infos-club\journee-type.html'),(Join-Path $dest 'apps\infos-club\tarifs.html'),(Join-Path $dest 'index.html'),(Join-Path $dest 'install.html'),(Join-Path $dest 'login.html')); foreach($f in $files){ if(Test-Path $f){ $c=[IO.File]::ReadAllText($f,[Text.UTF8Encoding]::new($false)) -replace '__BUILD_VERSION__',$bv; [IO.File]::WriteAllText($f,$c,[Text.UTF8Encoding]::new($false)) } }"
