...
powershell -NoProfile -Command "$bv='%LOGTS%'; $dest='%DEST%'; $files=@((Join-Path $dest 'apps\formations\index.html'),(Join-Path $dest 'apps\materiel\index.html'),(Join-Path $dest 'apps\infos-club\index.html'),(Join-Path $dest 'apps\infos-club\journee-type.html'),(Join-Path $dest 'apps\infos-club\tarifs.html'),(Join-Path $dest 'index.html'),(Join-Path $dest 'install.html')); foreach($f in $files){ if(Test-Path $f){ $c=[IO.File]::ReadAllText($f,[Text.UTF8Encoding]::new($false)) -replace '__BUILD_VERSION__',$bv; [IO.File]::WriteAllText($f,$c,[Text.UTF8Encoding]::new($false)) } }"
